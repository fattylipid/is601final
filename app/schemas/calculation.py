...
class CalculationType(str, Enum):
    ...
    DIVISION = "division"
    MODULUS = "modulus"  # <-- already present
...
class CalculationBase(BaseModel):
    ...
    def validate_inputs(self) -> "CalculationBase":
        """
        - Enforce at least two inputs
        - For division/modulus, ensure no divisor is zero
        """
        if len(self.inputs) < 2:
            raise ValueError("At least two numbers are required for calculation")
        if self.type in {CalculationType.DIVISION, CalculationType.MODULUS}:
            if any(x == 0 for x in self.inputs[1:]):
                msg = "Cannot divide by zero" if self.type == CalculationType.DIVISION else "Cannot perform modulus by zero"
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "examples": [
                ...
                {"type": "modulus", "inputs": [20, 7]}  # <-- added example
            ]
        }
    )
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": {
                ...
                "user_id": "123e4567-e89b-12d3-a456-426614174000",
                "type": "modulus",              # <-- show modulus in the response example
                "inputs": [20, 7],
                "result": 6.0,
